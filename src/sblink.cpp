...
void SLink::RegisterCoef(vmat_t& J, real_t w){
	uint i = con->index;
	uint j = var->index;
...
void X3Link::RegisterCoef(vmat_t& J, real_t w){
	uint i = con->index;
	uint j = var->index;
	J[i+0][j+0] =  0.0        ; J[i+0][j+1] = -w * coef[2]; J[i+0][j+2] =  w * coef[1];
	J[i+1][j+0] =  w * coef[2]; J[i+1][j+1] =  0.0        ; J[i+1][j+2] = -w * coef[0];
	J[i+2][j+0] = -w * coef[1]; J[i+2][j+1] =  w * coef[0]; J[i+2][j+2] =  0.0        ;
}	
...
void C2Link::RegisterCoef(vmat_t& J, real_t w){
	uint i = con->index;
	uint j = var->index;
	J[i+0][j] = w * coef[0];
	J[i+1][j] = w * coef[1];
...
void C3Link::RegisterCoef(vmat_t& J, real_t w){
	uint i = con->index;
	uint j = var->index;
	J[i+0][j] = w * coef[0];
	J[i+1][j] = w * coef[1];
...
void R2Link::RegisterCoef(vmat_t& J, real_t w){
	uint i = con->index;
	uint j = var->index;
	J[i][j+0] = w * coef[0];
	J[i][j+1] = w * coef[1];
...
void R3Link::RegisterCoef(vmat_t& J, real_t w){
	uint i = con->index;
	uint j = var->index;
	J[i][j+0] = w * coef[0];
	J[i][j+1] = w * coef[1];
...
void M2Link::RegisterCoef(vmat_t& J, real_t w){
	uint i = con->index;
	uint j = var->index;
	for(int ii = 0; ii < 2; ii++)for(int jj = 0; jj < 2; jj++)
		J[i+ii][j+jj] = w * coef[ii][jj];
}	
...
void M3Link::RegisterCoef(vmat_t& J, real_t w){
	uint i = con->index;
	uint j = var->index;
